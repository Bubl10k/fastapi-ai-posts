...
router = APIRouter(prefix="/comments/{comment_id}/responses",
                   tags=["Comment responses"], dependencies=[Depends(AuthService.get_current_user)])
...
async def create_comment_response(
    ...
    return await comment_response_service.create_comment_response(comment_response_create, user.id, comment_id)
